...
api.get("/teams/:id", (req, res) => {
  ...
  res.json({
    ...
    payments: { id: `tp${t.id}`, season: D.SEASON, amount: 3000, status: t.id === "t1" ? "PAID" : "PENDING", variableSymbol: `3000${t.id.slice(1)}` },
...
api.get("/payments/me", needAuth, (req, res) =>
  res.json({
    ...
    teamPayment: [{ id: "tp-t1", teamId: "t1", season: D.SEASON, amount: 3000, status: "PENDING", paidAt: null, variableSymbol: "3000001", team: D.teamLite("t1") }],
...
api.get("/payments/qr/:type/:id", needAuth, (req, res) => {
  ...
  if (req.params.type === "player-license") {
    ...
    // id = packId, prefix 7
    cfg = { vs: "7000001", amount: 2000, message: "FSL balicek 12 zapasu Tomas Novak" };
  }
...
api.get("/payments/bank-transactions", needAuth, (req, res) =>
  res.json([
    ...
    { id: "bt2", transactionId: "tx2", amount: 3000, variableSymbol: "3000001", senderName: "Benavidez Eagles z.s.", senderAccount: "987654321/0100", date: D.iso(-5), matched: false },
  ]),
);
...
api.get("/supervisor/payments", needAuth, (req, res) =>
  res.json({
    ...
    teams: D.TEAMS.map((t) => ({ id: `tp${t.id}`, season: D.SEASON, amount: 3000, status: t.id === "t1" ? "PAID" : "PENDING", variableSymbol: `3000${t.id.slice(1)}`, team: D.teamLite(t.id) })),
  }),
);
